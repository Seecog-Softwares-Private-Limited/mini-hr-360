...
const calculateStructureTotals = (components) => {
  ...
  components.forEach(c => {
    ...
    console.log(`Component: ${compName}, val=${val}, type=${typeField}, calcType=${calcType}, isDeduction=${isDeduction}, isBasic=${isBasic}`);
  ...
  console.log(`\nAfter loop: basicAmount=${basicAmount}, fixedEarnings=${fixedEarnings}`);
...
(async () => {
  try {
    const [rows] = await db.query(`SELECT * FROM salary_structures WHERE businessId = 26 LIMIT 5`);
    
    for (const row of rows) {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`Structure: ${row.name} (ID: ${row.id})`);
      console.log(`Raw components type: ${typeof row.components}`);
      ...
      if (typeof comps === 'string') {
        try {
          ...
          console.log(`Failed to parse components: ${e.message}`);
          comps = [];
        }
      }
      
      console.log(`Components count: ${Array.isArray(comps) ? comps.length : 'NOT AN ARRAY'}`);
      console.log(`Components data:`, JSON.stringify(comps, null, 2));
      
      const totals = calculateStructureTotals(comps);
