...
export const recalculateDay = async ({ businessId, employeeId, date, source = 'AUTO', notes = null, transaction = null }) => {
  ...
  else if (weekoff) status = 'WEEKOFF';
  else if (onLeave) status = 'LEAVE';
  else if (!firstIn && !lastOut) status = 'ABSENT';
  else if (workMinutes >= fullDayMins) status = 'PRESENT';
  else if (workMinutes >= halfDayMins) status = 'HALF_DAY';
  else status = 'ABSENT';
...
export const getDashboard = async ({ businessId, date }) => {
  ...
  const summaries = await AttendanceDailySummary.findAll({ where: { businessId, date: d } });
  ...
  return { date: d, counts, pendingRegularizations };
};
...
export const getAttendanceLogs = async ({ businessId, date, department = null, status = null }) => {
  ...
  const logs = await AttendanceDailySummary.findAll({
    where,
    include: [{ model: Employee, as: 'employee', attributes: ['id', 'empName', 'empDepartment', 'empDesignation'] }],
