...
export const postPolicy = asyncHandler(async (req, res) => {
  const businessId = await resolveBusinessId(req);
  const { name, rulesJson = {}, status = 'ACTIVE' } = req.body;
  if (!name) throw new ApiError(400, 'name is required');
  const policy = await createPolicy({ businessId, name, rulesJson, status });
...
export const patchPolicy = asyncHandler(async (req, res) => {
  ...
  const policy = await updatePolicy({ id: Number(id), businessId, patch: req.body });
  console.log(policy);
